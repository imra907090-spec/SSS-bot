...
# ✅ FIX #2: পুরো callback handler নতুন করে লেখা হলো — admin check, proper ordering, error handling সহ
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ FIX: Admin-only callbacks এর জন্য প্রথমেই চেক
    admin_only_prefixes = [
        "adm_menu_", "adm_usermgnt_", "adm_sys_",
        "manage_svc_view_", "manage_svc_country_view_", "manage_svc_del_range_",
        "manage_svc_edit_range_", "manage_svc_add_range_", "manage_svc_rename_",
        "manage_svc_del_country_", "manage_svc_delete_init_", "manage_svc_add",
        "manage_svc_back_to_list", "manage_svc_services"
    ]
    
    is_admin_callback = any(data.startswith(p) for p in admin_only_prefixes)
    
    if is_admin_callback and not is_admin(uid):
    ...
    try:
        ...
        # ✅ FIX: manage_svc_services আলাদা callback data — আগে manage_svc_add এর সাথে কনফ্লিক্ট হচ্ছিল
        elif data == "adm_menu_services" or data == "manage_svc_back_to_list":
            await query.message.edit_text(
        ...
        # ===== ADMIN — SERVICE MANAGEMENT (সবচেয়ে specific প্যাটার্ন আগে) =====
        elif data.startswith("manage_svc_edit_range_"):
            parts = data.replace("manage_svc_edit_range_", "").rsplit("_", 1)
            if len(parts) == 2:
            ...
            parts = data.replace("manage_svc_del_range_", "").rsplit("_", 1)
            if len(parts) == 2:
                prefix_and_country = parts[0]
                range_val = parts[1]
        ...
        # ✅ FIX: manage_svc_add সবচেয়ে শেষে রাখা হলো যাতে উপরের সব specific pattern আগে match হয়
        ...
        # ===== ADMIN — USER MANAGEMENT =====
            ...
            # ✅ FIX: যেকোনো unmatched callback এ নীরবে কিছু না করে জানানো হলো
            print(f"⚠️ Unmatched callback: {data} from user {uid}")
        ...
        try:
            await query.message.edit_text(
                ...
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 BACK", callback_data="adm_menu_back_to_admin")]])
            )
        except:
            pass

# ==================== MESSAGE HANDLER ====================
# ✅ FIX #3: সম্পূর্ণ message handler লেখা হলো — আগে এটি কাটা ছিল
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ FIX: ভ্যারিয়েবল ব্যবহার করে সরাসরি ম্যাচিং — কোনো rendering পার্থক্য থাকলেও কাজ করবে
    if text == ADMIN_BTN_TEXT:
        if is_admin(uid):
            await update.message.reply_text(
                get_admin_panel_text(),
                parse_mode="HTML",
    ...
    if text == PROFILE_BTN_TEXT:
        await profile_command(update, context)
        return

    if text == SUPPORT_BTN_TEXT:
        await support_command(update, context)
        return

    if text == LEADERBOARD_BTN_TEXT:
        await leaderboard_command(update, context)
        return

    if text == REFER_BTN_TEXT:
